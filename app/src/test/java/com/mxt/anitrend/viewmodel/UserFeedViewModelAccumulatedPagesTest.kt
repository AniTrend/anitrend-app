...
class UserFeedViewModelAccumulatedPagesTest {
    ...
    @Test
    fun `load page one then page two accumulates all loaded user feed items`() = runTest {
        stubPage(1, feed(id = 1L), feed(id = 2L))
        stubPage(2, feed(id = 3L), feed(id = 4L))
        val viewModel = createViewModel()

        viewModel.load(userId = 99, page = 1, pageLimit = 20, isFollowing = null, type = null, isMixed = null)
        viewModel.load(userId = 99, page = 2, pageLimit = 20, isFollowing = null, type = null, isMixed = null)
        advanceUntilIdle()
    ...
    @Test
    fun `mutation updates item from previously loaded user feed page`() = runTest {
        ...
        baseMutations.tryEmit(
            BaseMutation.LikeToggled(
                users = listOf(user(id = 8L, name = "test")),
                targetId = 1L,
    ...
    @Test
    fun `refresh replaces previously accumulated user feed pages`() = runTest {
        doReturn(
    ...
    private fun createViewModel() = UserFeedViewModel(
