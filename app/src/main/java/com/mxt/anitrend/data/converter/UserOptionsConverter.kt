...
class UserOptionsConverter : PropertyConverter<UserOptions, String> {
    override fun convertToEntityProperty(databaseValue: String?): UserOptions? {
        return databaseValue?.let { WebFactory.gson.fromJson(it, UserOptions::class.java) }
    }

    override fun convertToDatabaseValue(entityProperty: UserOptions?): String? {
