 ...
 */
public class AnimeStaffRoles extends RecyclerViewAdapter<Anime> {

    private ApiPreferences apiPrefs;

    public AnimeStaffRoles(List<Anime> mAdapter, Context mContext) {
        super(mAdapter, mContext);
        apiPrefs = new ApiPreferences(mContext);
    }

    @Override
    public RecyclerViewHolder<Anime> onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    private class ViewHolder extends RecyclerViewHolder<Anime> {
        ...
        @Override
        public void onBindViewHolder(Anime model) {
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.relation_model_image:
                    Anime model = mAdapter.get(getAdapterPosition());
