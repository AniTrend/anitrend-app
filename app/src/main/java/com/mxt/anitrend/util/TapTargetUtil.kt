...
object TapTargetUtil {
    ...
    fun showMultiplePrompts(vararg tapTargetPrompts: MaterialTapTargetPrompt.Builder?) {
        ...
        tapTargetPrompts.filterNotNull()
            .forEach { sequence.addPrompt(it.create()) }
        if (sequence.size() > 0)
            sequence.show()
    }

    fun buildDefault(context: FragmentActivity, @IdRes resource: Int): MaterialTapTargetPrompt.Builder =
     ...
     */
    fun isActive(@KeyUtil.TapTargetType key: String): Boolean =
        activePrompts.contains(key)

    /**
     * Adds or removes the key for a given tip
     */
    fun setActive(@KeyUtil.TapTargetType key: String, remove: Boolean) {
        if (!remove)
