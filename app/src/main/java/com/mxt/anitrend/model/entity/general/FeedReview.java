...
public class FeedReview implements Parcelable {

    private List<Review> anime;
    private List<Review> manga;

    public FeedReview(List<Review> anime, List<Review> manga) {
        this.anime = anime;
        this.manga = manga;
    }

    protected FeedReview(Parcel in) {
        anime = in.createTypedArrayList(Review.CREATOR);
        manga = in.createTypedArrayList(Review.CREATOR);
    }

    public static final Creator<FeedReview> CREATOR = new Creator<FeedReview>() {
        @Override
        public FeedReview createFromParcel(Parcel in) {
            return new FeedReview(in);
        }

        @Override
        public FeedReview[] newArray(int size) {
            return new FeedReview[size];
        }
    };
