...
public abstract class FragmentReviewBase extends FragmentBase<Review, BasePresenter, FeedReview> implements
    ...
    protected FeedReview model;
    protected String query;
    protected boolean isLimit;

    protected RecyclerViewAdapter<Review> mAdapter;
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        if(getPresenter() != null && key.equals(ApplicationPref.KEY_REVIEW_TYPES)) {
    ...
    public void onSearch(String query) {
        if(isAlive() && mAdapter != null && !isPager) {
            this.query = query;
    ...
    protected void injectAdapter() {
        if(model != null) {
            ...
            if (recyclerView.getAdapter() == null) {
                ...
                mAdapter.onItemsInserted(getPresenter().getApplicationPref().getReviewType() ? model.getAnime() : model.getManga());
                recyclerView.setAdapter(mAdapter);
            } else {
                List<Review> reviewList = getPresenter().getApplicationPref().getReviewType() ? model.getAnime() : model.getManga();
                if (swipeRefreshLayout.isRefreshing()) {
                    swipeRefreshLayout.setRefreshing(false);
                    mAdapter.onItemsInserted(reviewList);
                }
                else {
                    if(swipeRefreshLayout.isLoading())
                        swipeRefreshLayout.setLoading(false);
                    mAdapter.onItemRangeInserted(reviewList);
                }
     ...
     */
    @Override
    public void onChanged(@Nullable FeedReview content) {
        if(content != null) {
            if(content.getAnime().size() > 0 || content.getManga().size() > 0) {
                if(isPager) {
                    if (model == null)
                        model = FilterProvider.getReviewFilter(getPresenter().getDatabase().getCurrentUser(), content);
                    else {
                        model.setAnime(FilterProvider.getReviewFilter(getPresenter().getDatabase().getCurrentUser(), content.getAnime()));
                        model.setManga(FilterProvider.getReviewFilter(getPresenter().getDatabase().getCurrentUser(), content.getManga()));
                    }
                }
                else
                    model = content;
     ...
     */
    @Override
    public abstract void onItemClick(View target, Review data);
     ...
     */
    @Override
    public abstract void onItemLongClick(View target, Review data);
}