            ...
            try {
                switch (mActionType) {
                    case ACTIVITY_CREATE:
                        ...
                        return (Call<T>) create.createActivityStatus(EmojiUtils.hexHtmlify(genericAction.getText()));
                    case ACTIVITY_EDIT:
                        UserModel edit = ServiceGenerator.createService(UserModel.class, mContext);
                        return (Call<T>) edit.activityEdit(genericAction.getId(), EmojiUtils.hexHtmlify(genericAction.getText()), genericAction.getUser_id());
                    case ACTIVITY_REPLY:
                        UserModel reply = ServiceGenerator.createService(UserModel.class, mContext);
                        return (Call<T>) reply.createActivityReply(genericAction.getReply_id(), EmojiUtils.hexHtmlify(genericAction.getText()));
                    case ACTIVITY_REPLY_EDIT:
                        UserModel reply_edit = ServiceGenerator.createService(UserModel.class, mContext);
                        return (Call<T>) reply_edit.activityEditReply(genericAction.getId() , genericAction.getReply_id(), EmojiUtils.hexHtmlify(genericAction.getText()));
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            try {
                switch (mActionType) {
                    case DIRECT_MESSAGE_SEND:
                        ...
                        return (Call<T>) message.createActivityMessage(EmojiUtils.hexHtmlify(messageAction.getText()), messageAction.getMessenger_id());
                    case DIRECT_MESSAGE_EDIT:
                        UserModel message_edit = ServiceGenerator.createService(UserModel.class, mContext);
                        return (Call<T>) message_edit.activityMessageEdit(messageAction.getId(), messageAction.getMessenger_id(), EmojiUtils.hexHtmlify(messageAction.getText()), messageAction.getUser_id());
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
