...
class SeriesYearTypeTextView @JvmOverloads constructor(
    context: Context,
    ...
    companion object {
        @JvmStatic
        fun htmlText(seriesYearTypeTextView: SeriesYearTypeTextView, mediaBase: MediaBase) {
            val context = seriesYearTypeTextView.context
            val startDate: FuzzyDate? = mediaBase.startDate
            val year = if (startDate?.isValidDate == true)
            ...
            when (mediaBase.type) {
                KeyUtil.ANIME ->
                    if (CompatUtil.equals(mediaBase.format, KeyUtil.MOVIE))
                        seriesYearTypeTextView.text = String.format(
                            Locale.getDefault(),
                            "%s - %s",
                            year,
