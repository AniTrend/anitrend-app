 ...
 */
public class OnHoldMangaFragment extends UserListFragment {

    public OnHoldMangaFragment() {
        // Required empty public constructor
    }

    public static UserListFragment newInstance(int id) {
        Bundle bundle = new Bundle();
        OnHoldMangaFragment mFragment = new OnHoldMangaFragment();
        ...
        if (isVisible() && (!isDetached() || !isRemoving()))
            if (response.isSuccessful() && response.body() != null)
                if(response.body().getLists() == null)
                    ...
                    sortItems(response.body().getLists().getOn_hold());
            else
                showError(response);
    }
}