 ...
 */
public class CompletedAnimeFragment extends UserListFragment {

    public CompletedAnimeFragment() {
        // Required empty public constructor
    }

    public static UserListFragment newInstance(int id) {
        Bundle bundle = new Bundle();
        CompletedAnimeFragment mFragment = new CompletedAnimeFragment();
        ...
        if (isVisible() && (!isDetached() || !isRemoving()))
            if (response.isSuccessful() && response.body() != null)
                if(response.body().getLists() == null)
                    ...
                    sortItems(response.body().getLists().getCompleted());
            else
                showError(response);
    }
}