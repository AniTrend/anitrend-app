 ...
 */
class MediaFavouriteFragment :
    FragmentBaseList<MediaBase, ConnectionContainer<Favourite>, BasePresenter>() {
    ...
    override fun makeRequest() {
        ...
        val requestMode = if (CompatUtil.equals(mediaType, KeyUtil.ANIME))
            KeyUtil.USER_ANIME_FAVOURITES_REQ else KeyUtil.USER_MANGA_FAVOURITES_REQ
    ...
    override fun onChanged(content: ConnectionContainer<Favourite>?) {
        if (content != null) {
            if (!content.isEmpty) {
                val pageContainer = if (CompatUtil.equals(mediaType, KeyUtil.ANIME))
                    content.connection.anime else content.connection.manga
