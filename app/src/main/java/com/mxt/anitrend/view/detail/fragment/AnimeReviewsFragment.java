...
public class AnimeReviewsFragment extends Fragment implements Callback<List<Review>>, ReviewClickListener {

    @BindView(R.id.detail_review_recycler) RecyclerView mRecyclerReview;
    @BindView(R.id.review_container) ProgressLayout progressLayout;
    ...
    public static AnimeReviewsFragment newInstance(int id) {
        Bundle args = new Bundle();
        args.putInt(ARG_KEY, id);
        AnimeReviewsFragment fragment = new AnimeReviewsFragment();
    ...
    private void setupRecyclers() {
        mRecyclerReview.setLayoutManager(new LinearLayoutManager(getContext()));
    ...
    private void makeReviewsRequest() {
        new AsyncTaskFetch<>(this, getContext(), item_id).execute(AsyncTaskFetch.RequestType.ANIME_REVIEWS_REQ);
    }
    ...
    public void onReadMore(int index) {
        ...
        new DialogManager(getContext()).createDialogMessage(String.format(Locale.getDefault(), getString(R.string.text_reviewed_by), review.getUser().getDisplay_name()), review.getText());
    }
    ...
    public void onUpvote(int index) {
        if(!prefs.isAuthenticated()) {
            Toast.makeText(getContext(), "Please sign into the app first!", Toast.LENGTH_SHORT).show();
            return;
        }
        Review review = reviews.get(index);
        ...
        }, FilterTypes.ActionType.REVIEW_ANIME_RATE, payload).execute();
    }
