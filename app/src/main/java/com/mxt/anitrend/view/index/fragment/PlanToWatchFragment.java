 ...
 */
public class PlanToWatchFragment extends UserListFragment {
    ...
    public static UserListFragment newInstance(int id) {
        ...
        PlanToWatchFragment mFragment = new PlanToWatchFragment();
        ...
        if (isVisible() && (!isDetached() || !isRemoving()))
            if (response.isSuccessful() && response.body() != null)
                if(response.body().getLists() == null)
                    ...
                    sortItems(response.body().getLists().getPlan_to_watch());
            else
                showError(response);
    }
}
