...
fun StudioBaseData.toStudioEntity(): StudioEntity = studio?.toStudioEntity() ?: throw IllegalStateException("Empty response body")

fun StudioBaseData.Studio.toStudioEntity(): StudioEntity = StudioEntity().also { entity ->
    entity.id = id.toLong()
    entity.name = name
...
fun StudioMediaData.StudioMedia.toStudioMediaPage(): PageContainer<MediaEntity> = PageContainer<MediaEntity>().also { page ->
...
fun StudioMediaData.StudioMediaNodes.toMediaEntity(): MediaEntity = MediaEntity().also { entity ->
    entity.id = id.toLong()
...
private fun StudioMediaData.StudioMediaNodesTitle.toMediaTitle(): MediaTitle = MediaTitle(
    romajiRaw = romaji,
    englishRaw = english,
    originalRaw = native,
...
private fun StudioMediaData.StudioMediaNodesCoverImage.toImageBase(): ImageBase = ImageBase(
    extraLarge = extraLarge,
    large = large,
    medium = medium,
)

private fun StudioMediaData.StudioMediaNodesStartDate.toFuzzyDate(): FuzzyDate = FuzzyDate(
    day = day ?: 0,
    month = month ?: 0,
    year = year ?: 0,
)

private fun StudioMediaData.StudioMediaNodesEndDate.toFuzzyDate(): FuzzyDate = FuzzyDate(
    day = day ?: 0,
    month = month ?: 0,
    year = year ?: 0,
)

private fun StudioMediaData.StudioMediaNodesNextAiringEpisode.toAiringSchedule(): AiringSchedule = AiringSchedule(
    airingAt = airingAt.toLong(),
    timeUntilAiring = timeUntilAiring.toLong(),
    episode = episode,
)

private fun StudioMediaData.StudioMediaNodesMediaListEntry.toMediaList(): MediaList = MediaList().also { mediaList ->
    mediaList.id = id.toLong()
    mediaList.status = status?.name
}

private fun StudioMediaData.StudioMediaPageInfo.toPageInfo(): PageInfo = PageInfo(
    total = total ?: 0,
    perPage = perPage ?: 0,
